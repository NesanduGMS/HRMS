...
export const getTotalEmployees = (req, res) => {
    db.query('CALL getTotalEmployees()', (error, results) => {
        if (error) return res.status(500).json({ error });
        res.json({ totalEmployees: results[0][0].totalEmployees });
    });
};

export const getLeavesTaken = (req, res) => {
    db.query('CALL getLeavesTaken()', (error, results) => {
        if (error) return res.status(500).json({ error });
        res.json({ leavesTaken: results[0][0].leavesTaken });
    });
};

export const getPendingLeaveAppeals = (req, res) => {
    db.query('CALL getPendingLeaveAppeals()', (error, results) => {
        if (error) return res.status(500).json({ error });
        res.json({ pendingLeaveAppeals: results[0][0].pendingLeaveAppeals });
    });
};

export const getAveragePerformance = (req, res) => {
    db.query('CALL getAveragePerformance()', (error, results) => {
        if (error) return res.status(500).json({ error });
        res.json({ averagePerformance: results[0][0].averagePerformance });
    });
};
